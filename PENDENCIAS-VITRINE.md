...
| **Logotipos das montadoras** | `frontend/src/storefront/brandLogos.tsx` | Nissan e Renault usam o vetor oficial (fornecido pela loja). Fiat, Jeep, BYD, Suzuki, Chery, Peugeot e Mitsubishi foram redesenhados a partir da referência visual enviada. As demais (Chevrolet, Honda, Toyota, VW, Ford, Hyundai, Citroën, BMW, Mercedes, Audi) seguem como **interpretações geométricas próprias**, não os logotipos registrados. Se a loja enviar o arquivo vetorial oficial de alguma marca, é só trocar o `d` do path daquela marca. Marca sem emblema cai no monograma tipográfico automaticamente. |
